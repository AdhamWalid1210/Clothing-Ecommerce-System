...
def test_view_cart():
    ...
    try:
        ...
        # Step 2: Add a product to the cart (optional, if the cart is empty)
        ...
        # Verify that the cart section is displayed
        cart_section = driver.find_element(By.ID, "cart-items")
        assert cart_section.is_displayed(), "Cart section is not displayed"

        # Verify that the total amount is displayed
        total_amount = driver.find_element(By.ID, "total-amount")
        assert total_amount.is_displayed(), "Total amount is not displayed"

        print("Test Case 3: View Cart Items - Passed")
